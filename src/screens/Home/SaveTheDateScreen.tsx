...
export default function SaveTheDateScreen({ navigation }: any) {
    ...
    const handleShare = async () => {
        try {
            const cacheKey = `save_the_date_${partner1Name}_${partner2Name}.png`;
            const uri = await shareService.captureShareCard(viewShotRef, cacheKey);
            await shareService.shareGeneral(uri, `Save the Date! ${partner1Name} & ${partner2Name} are getting married on ${formattedDate}.`);
        } catch (error: any) {
